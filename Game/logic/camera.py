...
class Camera:
    ...
    def apply(self, rect):
        ...
        Args:
            rect (pygame.Rect): Прямоугольник, к которому нужно применить смещение.

        Returns:
            pygame.Rect: Новый прямоугольник с примененным смещением.
        """
        return rect.move(-self.offset.x, -self.offset.y)

    def update(self, target):
        """
        Обновляет позицию камеры, центрируя её на цели (игроке).

        Args:
            target (pygame.sprite.Sprite): Цель, за которой следит камера.
        """
